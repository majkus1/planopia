			...
			<Router>
				<div>
					<Routes>
						...
						<Route element={<ProtectedRoute isLoggedIn={loggedIn} token={token} />}>
							...
							<Route
								...
								element={
									[
										...
										'Kierownik Marketing',
										'Urlopy czas pracy',
									].some(role => JSON.parse(localStorage.getItem('roles') || '[]').includes(role)) ? (
							...
							<Route
								path='/logs'
								element={
									['Admin'].some(role => JSON.parse(localStorage.getItem('roles') || '[]').includes(role)) ? (
